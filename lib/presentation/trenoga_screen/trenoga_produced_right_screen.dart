...
class TrenogaProducedRightScreen extends StatelessWidget {
  const TrenogaProducedRightScreen({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    context.select((TrenogaDataViewModel vm) => vm.producedTrenoga);
    context.select((TrenogaDataViewModel vm) => vm.additionTrenogaCalculateNumber);
    Map<String, SpendMaterial> materialProduceMap =
        TrenogaDataViewModel.instance.getMaterialProduceMap();
...
class _AdditionDataListViewState extends State<AdditionDataListView> {
  ...
  void initState() {
    ...
    additionTextController.text =
        TrenogaDataViewModel.instance.additionTrenogaCalculateNumber.toString();
    additionTextController.addListener(() {
      TrenogaDataViewModel.instance
  ...
  Widget build(BuildContext context) {
    int additionCalculateNumber =
        context.select((TrenogaDataViewModel vm) => vm.additionTrenogaCalculateNumber);
    Map<String, int> additionCalculateMap = TrenogaDataViewModel.instance.getAdditionCalculateMap();
          ...
          child: Column(
            children: [
              ...
              Padding(
                ...
                child: Text('Дополнительно: ', style: const TextStyle(fontSize: 16)),
              ),
              buildUserInputRow(),
              SizedBox(height: 15),
              Flexible(
  ...
  Widget buildUserInputRow() {
    return Row(
      children: [
        ...
        Flexible(flex: 7, fit: FlexFit.tight, child: SizedBox()),
      ],
    );
  }
  ...
  void initState() {
    ...
    textController.addListener(() {
      if (widget.spendMaterial.historySupply != textController.text) {
        TrenogaDataViewModel.instance
            .saveHistoryTrenogaSupply(name: widget.title, data: textController.text);
      }
      setState(() {});
    });
  }
