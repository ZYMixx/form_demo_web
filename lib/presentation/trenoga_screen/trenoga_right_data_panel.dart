...
import 'package:form_demo_web/presentation/trenoga_screen/trenoga_produced_right_screen.dart';
...
class TrenogaRightDataPanel extends StatefulWidget {
  const TrenogaRightDataPanel({Key? key}) : super(key: key);

  @override
  State<TrenogaRightDataPanel> createState() => _TrenogaRightDataPanelState();
}

class _TrenogaRightDataPanelState extends State<TrenogaRightDataPanel>
  ...
  void _handleTabSelection() {
    if (_tabController.indexIsChanging) {
      if (_tabController.index == 0) {
        TrenogaDataViewModel.instance.changeTrenogaTab(tab: TrenogaTabEnum.rentability);
      }
      if (_tabController.index == 1) {
        TrenogaDataViewModel.instance.changeTrenogaTab(tab: TrenogaTabEnum.produced);
      }
    }
  }
  ...
  void initState() {
    super.initState();
    _tabController = TabController(length: 2, vsync: this); // указываем количество вкладок
    _tabController.addListener(_handleTabSelection);
    switch (TrenogaDataViewModel.instance.selectedTab) {
          ...
          children: [
            ...
            Flexible(
              child: TabBarView(
                controller: _tabController,
                children: [
                  CountTrenogaDataWidget(),
                  ProducedTrenogaDataWidget(),
                ],
              ),
            ),
          ],
...
class CountTrenogaDataWidget extends StatelessWidget {
  const CountTrenogaDataWidget({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    TrenogaCalculateItem trenogaCalculate =
        context.select((TrenogaDataViewModel vm) => vm.trenogaCalculateItem);
    int targetRentabel = context.select((TrenogaDataViewModel vm) => vm.targetRentability);
    return Padding(
          ...
          Flexible(
            ...
            child: Column(
              children: [
                ...
                buildShowDataItem(
                  title: "Цена 1-шт",
                  value: ToolDoubleRound.round(number: trenogaCalculate.getSumCostWithOutVat()),
                  height: 80,
                  numAnimation: true,
                ),
                buildShowDataItem(
                  title: "Цена **с НДС**",
                  value:
                      ToolDoubleRound.round(number: trenogaCalculate.getSumCostWithOutVat() * 1.20),
                  height: 80,
                  numAnimation: true,
                ),
                ...
                buildShowDataItem(
                  ...
                  value: ToolDoubleRound.round(
                    number: trenogaCalculate.getSumCostWithVat() * ((100 + targetRentabel) / 100),
                  ),
                  height: 80,
                  numAnimation: true,
                ),
                for (int numRent in [5, 10, 15, 20])
                  buildShowDataItem(
                    title: "**$numRent**-%",
                    value: ToolDoubleRound.round(
                      number: trenogaCalculate.getSumCostWithVat() * ((100 + numRent) / 100),
                    ),
                    height: 60,
                    numAnimation: true,
                  ),
                ...
                child: SingleChildScrollView(
                  child: Column(
                    children: [
                      ...
                      buildDataInfoColumn(
                          dataMap: trenogaCalculate.materialCostMap,
                          summ: trenogaCalculate.sumMaterial),
                      buildDataInfoColumn(
                          dataMap: trenogaCalculate.salaryWorkCostMap,
                          summ: trenogaCalculate.sumSalaryWork),
                      buildDataInfoColumn(
                          dataMap: trenogaCalculate.otherCostCostMap,
                          summ: trenogaCalculate.sumOtherCost),
                      MyWidgetButton(
                        onPressed: () {
                          TableViewModel.instance.chaneSelectedDBTable('Материалы*');
                          TableViewModel.instance.expansionIsOpen[4] = true;
...
class ProducedTrenogaDataWidget extends StatelessWidget {
  const ProducedTrenogaDataWidget({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return TrenogaProducedRightScreen();
  }
}
  ...
  return Padding(
    ...
    child: Row(
      children: [
        ...
        if (listValue == null && mapValue != null && trenogaCount != null)
          for (var value in mapValue.values)
            buildShowDataBox(height, numAnimation, value * trenogaCount),
        if (listValue != null)
...
class MyNumberAnimation extends StatefulWidget {
  ...
  MyNumberAnimation({required this.targetNumber, required this.height, required this.duration});

  @override
  _MyNumberAnimationState createState() => _MyNumberAnimationState();
}

class _MyNumberAnimationState extends State<MyNumberAnimation> with SingleTickerProviderStateMixin {
